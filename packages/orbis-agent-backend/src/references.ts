...
import { createAgentSessionRef, type AgentSessionRef } from "./identifiers";

export type AgentPromptReferenceSource = "files" | "sessions";

export interface AgentPromptReferenceCompletionInput {
...
function candidate(value: unknown): AgentPromptReferenceCandidate {
  ...
  return {
    ...(input.detail === undefined
      ? {}
      : { detail: display(input.detail, "Reference candidate detail", AGENT_PROMPT_REFERENCE_MAX_DETAIL_LENGTH) }),
    insertText: display(
      input.insertText,
    ...
    label: display(input.label, "Reference candidate label", AGENT_PROMPT_REFERENCE_MAX_LABEL_LENGTH),
  };
}
...
export function validateAgentPromptReferenceCompletionInput(
  ...
  const textValue = text(input.text, "Reference completion text", AGENT_PROMPT_REFERENCE_MAX_TEXT_LENGTH, true);
...
export function validateAgentPromptReferenceCompletionResult(
  ...
  if (!Array.isArray(candidatesValue) || candidatesValue.length > AGENT_PROMPT_REFERENCE_MAX_CANDIDATES) {
  ...
  if (input !== undefined) {
    ...
    if (candidates.length > input.limit) return invalid("Reference completion exceeds the requested limit");
    const expectedKind = input.source === "files" ? new Set(["file", "directory"]) : new Set(["session"]);
  ...
  if (insertions.size !== candidates.length) return invalid("Reference completion candidates must be unique");
